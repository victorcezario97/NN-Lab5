...
activationsPooled = reshape(activationsPooled, ...
    [size(activationsPooled,1)*size(activationsPooled,2)*size(activationsPooled,3), numImages]);
...
Y_num = Wd * activationsPooled;
Y_num = bsxfun(@plus, Y_num, bd);
Y_num = exp(Y_num);

Y_den = sum(Y_num, 1);

probs = bsxfun(@rdivide, Y_num, Y_den);
